...
public class MineField extends JLabel implements MouseListener
{
  ...
  * ============================================= */
  ...
  public MineField( MineSweeper game )
  {
    ...
    squareLength = MineSweeper.MED_DIM ;
    setup( MineSweeper.MED_DIM );
    
    setFont( game.fontMEDIUM );
    setLocation( MineSweeper.XBORDER, MineSweeper.YBORDER/2 );
  ...
  protected void clearQmarks()
  {
    ...
    for( i=0; i<fieldLength; i++ )
      ...
      {
        if( mineArray[i][j].hasQmark() )
        {
          if( DEBUG_LEVEL > 1 )
            System.out.println( "MineField.clearQmarks(): Square[" + i + "][" + j
                                + "] has a question mark" );
          
          mineArray[i][j].setQmark( false );
          haveQmarks = true ;
        }
      }
  ...
  {
    int i, j ;
    mineArray = new Square[ fieldLength ][ fieldLength ];
    
    for( i=0; i<fieldLength; i++ )
      for( j=0; j<fieldLength; j++ )
      {
        mineArray[i][j] = new Square();
      ...
      for( j=0; j<fieldLength; j++ , detected=0 )
      {
        ...
        for( u=left; u <= right; u++ )
          for( v=top; v <= bottom; v++ )
            if( mineArray[u][v].hasMine() )
              detected++ ;
        
        mineArray[i][j].setMinecount( detected );
      }
  ...
  {
    if( !mineArray[u][v].isRevealed() )
      mineArray[u][v].setRevealed();
    ...
    for( i=left; i <= right; i++ )
      for( j=top; j <= bottom; j++ )
        if( !mineArray[i][j].isRevealed() )
        {
          mineArray[i][j].setRevealed();
          setScore( PLUS );
          // recursive call if any adjacent squares are also blank
          if( mineArray[i][j].isBlank() )
            clearOut( i, j ); 
        }
  ...
  public void mousePressed( MouseEvent evt )
  {
    ...
    Square activeSqr = mineArray[x][y] ;
  ...
  public void paint( Graphics page )
  {
    ...
    for( i=paintLeft; i <= paintRight; i++ )
      ...
      {
        if( mineArray[i][j].isRevealed() )
          reveal( i*squareLength, j*squareLength, mineArray[i][j].getMinecount(), page );
        else if( mineArray[i][j].hasFlag() )
          reveal( i*squareLength, j*squareLength, FLAG, page );
        else if( mineArray[i][j].hasQmark() )
  ...
  protected void drawSquare( int xc, int yc, Graphics page )
  {
    ...
    page.setColor( COLOR_LIGHT );
    page.drawLine( xc, yc, xc+squareLength-1, yc        ); // top
    page.drawLine( xc, yc, xc,        yc+squareLength-1 ); // left
  ...
  {
    page.fillOval( xc+(squareLength/2),    yc+(squareLength/8),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(7*squareLength/10), yc+(squareLength/2),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(2*squareLength/10), yc+(7*squareLength/10), squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/3),    yc+(3*squareLength/5),  squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/4),    yc+(squareLength/2),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/8),    yc+(squareLength/8),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/4),    yc+(2*squareLength/3),  squareLength/10, squareLength/10 );
    page.fillOval( xc+(7*squareLength/10), yc+(2*squareLength/5),  squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/2),    yc+(squareLength/5),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(3*squareLength/4),  yc+(squareLength/2),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/3),    yc+(squareLength/3),    squareLength/3,  squareLength/3  );
    page.fillOval( xc+(3*squareLength/5),  yc+(squareLength/3),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(2*squareLength/5),  yc+(2*squareLength/5),  squareLength/10, squareLength/10 );
    page.fillOval( xc+(squareLength/2),    yc+(squareLength/5),    squareLength/10, squareLength/10 );
    page.fillOval( xc+(2*squareLength/5),  yc+(1*squareLength/5),  squareLength/10, squareLength/10 );
    page.fillOval( xc+(3*squareLength/5),  yc+(4*squareLength/5),  squareLength/10, squareLength/10 );
